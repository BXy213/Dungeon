...
@export var wave_width: float = 120.0  # 波动宽度
...
func execute_skill_effect(target_position: Vector2, _target_node: Node) -> void:
	...
	# 创建震荡波
	var shockwave = create_skill_effect("sonic_wave", player.global_position)  # 复用sonic_wave的逻辑
	...
	shockwave.modulate = Color(0.3, 0.9, 0.9, 0.7)
