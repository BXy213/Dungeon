...
func attempt_interaction() -> void:
	...
	if distance > interaction_distance:
		print("⚠️ 距离宝箱太远了！（", int(distance), " / ", interaction_distance, "）")
		return
...
func try_open(interacting_player: Node) -> void:
	...
	if not interacting_player.has_method("remove_silver_key"):
		print("⚠️ 玩家没有remove_silver_key方法！")
		return
	
	if interacting_player.silver_key_count <= 0:
		print("🔑 银钥匙不足！需要至少1把银钥匙才能开启宝箱")
		return
...
func open_chest() -> void:
	...
	var sprite = get_node_or_null("Sprite2D")
	if sprite:
		var opened_texture = load("res://art/chestopened.png")
		if opened_texture:
